...
SComatic needs a TSV metadata file that relates each cell barcode with each "Cell type" annotation. The annotation will depend on how much granularity we need or is adequate to cal lsomatic variants from single cell transcriptomics. Here, the library design plays a very important role. Due to that, in our case, we have 8 libraries, that are truly 16 as they are double-sequenced. Each of the library contains 3 mice, and, we are not interested in finding intra-cell-type in the esophagus epithelium, also, all of the cells are epihtelial, so we don't have the possibility of cross-relating detected variants across different cell types of other lineages. That's why we have decided to annotate all the cell barcodes as "Epithelial". 
